...
> {
    ...
    let pool_hashes_vec = bytes.as_ref().ok_or(RouterError::MissingPoolHashes)?;
    
    // path should have at least 2 elements. ifnot error WrongMinimumPathLength
    if path.len() < 2 {
        return Err(RouterError::WrongMinimumPathLength);
    }
    ...
    if pool_hashes_vec.len() != path.len().checked_sub(1).unwrap() { // unwrap safe as we checked the length of path
        return Err(RouterError::WrongPoolHashesLength);
    }
